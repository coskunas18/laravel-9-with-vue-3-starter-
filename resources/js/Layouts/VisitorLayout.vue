            ...
            <div class=" flex bg-sky-300 dark:bg-gray-600 w-full h-12 items-center justify-between">
                ...
                <div class="flex items-center justify-center">
                    ...
                    <font-awesome-icon @click="menu.toggleMenu()" icon="fa-solid fa-bars" size="xl"
                        class="cursor-pointer pl-3 dark:text-white ml-3" />
                ...
                <div class=" flex space-x-5 items-center justify-center ">
                    ...
                    <div v-if="!$page.props.user">
                        <Link :href="route('login')"
                            class="text-sm text-gray-700 dark:text-slate-200 no-underline select-none">
                        {{ $t('auth.login') }}
                        </Link>
                        <Link v-if="canRegister" :href="route('register')"
                            class="ml-4 text-sm text-gray-700 dark:text-slate-200 no-underline select-none">
                        {{ $t('auth.register') }}
                        </Link>
                    ...
                    <div>
                        ...
                        <p style="margin: 0;padding: 0;" v-if="showLangs">
                            <template v-for="i in langs">
                                <span @click="changeLang(i.id)" v-if="(locale !== i.id)"
